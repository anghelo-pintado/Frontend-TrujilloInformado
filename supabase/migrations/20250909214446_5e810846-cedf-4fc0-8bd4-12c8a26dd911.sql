-- Schema for Urban Clean Hub MVP
-- 1) Enums
...
create policy if not exists "Users can set their own role (MVP)"
  on public.user_roles for insert to authenticated
  with check (user_id = auth.uid());

create policy if not exists "Users can update their own roles (MVP)"
...
-- zones policies
create policy if not exists "Zones are viewable by authenticated"
  on public.zones for select to authenticated using (true);

create policy if not exists "Supervisors can manage their zones"
  on public.zones for insert to authenticated
  with check (supervisor_id = auth.uid());

create policy if not exists "Supervisors can update their zones"
...
-- reports policies
create policy if not exists "Citizens, workers and zone supervisors can view relevant reports"
...
create policy if not exists "Citizens can create their own reports"
  on public.reports for insert to authenticated
  with check (
    citizen_id = auth.uid() and public.has_role(auth.uid(), 'ciudadano')
  );

create policy if not exists "Authorized users can update relevant reports"
...
-- tasks policies
create policy if not exists "Workers and supervisors can view tasks"
...
create policy if not exists "Supervisors can create tasks"
  on public.tasks for insert to authenticated
  with check (
    supervisor_id = auth.uid() and public.has_role(auth.uid(), 'supervisor')
  );

create policy if not exists "Workers or supervisors can update tasks"
...
-- evidences policies
create policy if not exists "Workers and supervisors can view evidences of their tasks"
...
create policy if not exists "Assigned worker can upload evidences"
...
create policy if not exists "Public can view report images"
  on storage.objects for select
  using (bucket_id = 'reports');

create policy if not exists "Users can upload report images to their folder"
  on storage.objects for insert to authenticated
  with check (
    bucket_id = 'reports' and auth.uid()::text = (storage.foldername(name))[1]
  );

create policy if not exists "Users can update their report images"
...
create policy if not exists "Users can view their own evidence images"
  on storage.objects for select to authenticated
  using (
    bucket_id = 'evidences' and auth.uid()::text = (storage.foldername(name))[1]
  );

create policy if not exists "Workers can upload evidence images to their folder"
  on storage.objects for insert to authenticated
  with check (
    bucket_id = 'evidences' and auth.uid()::text = (storage.foldername(name))[1]
  );

create policy if not exists "Workers can update their evidence images"
  on storage.objects for update to authenticated
  using (
    bucket_id = 'evidences' and auth.uid()::text = (storage.foldername(name))[1]
  )
