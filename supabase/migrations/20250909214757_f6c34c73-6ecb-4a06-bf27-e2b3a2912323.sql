-- Schema for Urban Clean Hub MVP (missing tables only)
-- 1) Utility function for updated_at (only if not exists)
...
create table if not exists public.user_roles (
  ...
  role text not null check (role in ('ciudadano','supervisor','trabajador')),
...
-- 3) Profiles and trigger on auth.users
...
create table if not exists public.reports (
  ...
  type text not null check (type in ('barrido','residuos_solidos','maleza')),
  description text,
...
-- 7) Evidences (photos linked to tasks)
